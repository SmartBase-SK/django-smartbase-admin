...
class SBAdminFormFieldWidgetsMixin:
    ...
    def assign_widget_to_form_field(self, form_field, db_field=None, request=None):
        ...
        if isinstance(form_field, RichTextFormField):
            kwargs["config_name"] = getattr(db_field, "config_name", None) or getattr(
                form_field.widget, "config_name", "default"
            )
            kwargs["external_plugin_resources"] = getattr(
                db_field, "external_plugin_resources", None
            ) or getattr(form_field.widget, "external_plugin_resources", [])
            kwargs["extra_plugins"] = getattr(
                db_field, "extra_plugins", None
            ) or getattr(form_field.widget, "extra_plugins", [])
