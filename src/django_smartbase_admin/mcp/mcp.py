...
@global_mcp_server.resource(
    ...
    description=(
        ...
        "SETUP: (1) save the content as an .html file; (2) serve it over http, "
        "NOT file:// (Origin 'null' is always CORS-blocked) — locally on a "
        "spare, non-default port so it doesn't clash with the admin/other dev "
        "servers, e.g. python -m http.server 8010; (3) open it and Connect — "
        "the OAuth client self-registers via DCR on first connect (a public "
        "PKCE id cached in localStorage; nothing to paste), and CONFIG.scope + "
        "the MCP path are prefilled; (4) build the "
        ...
        "off-origin (incl. a local http://localhost page against an https "
        "admin), point CONFIG.issuer/mcpUrl at this server and have the "
        "operator add the page's origin to SBADMIN_MCP_ALLOWED_ORIGINS with "
        "SBAdminMCPCorsMiddleware enabled. Plain http on localhost is fine — "
        "loopback is a secure context (PKCE works), the self-registered "
        "redirect_uri is loopback so http is accepted, and the admin traffic is "
        "still https."
    ),
    mime_type="text/html",
)
